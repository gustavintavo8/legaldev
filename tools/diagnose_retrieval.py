...
def diag_p3(vs: Chroma):
    ...
    probes = [
        ...
        (
            "plantas (probe 8 original)",
            "red social para amantes de las plantas donde la gente comparte fotos y consejos",
            {},
        ),
    ...
    print(
        f"{'Label':<45} {'RGPD top score':>15}  {'>=0.40?':>6}  {'RGPD injected?':>14}"
    )
    print("-" * 82)
    for r in results:
...
def diag_p2(vs: Chroma):
    ...
    print("\n" + "=" * 72)
    print(
        "P2 SUMMARY -- RGPD chunk count in final docs (prompt requires >=2 for section)"
    )
    print(
        f"{'Case':<45} {'RGPD chunks':>12}  {'in normativas_det':>18}  {'section?':>9}"
    )
    print("-" * 87)
    for r in results:
    ...
    print(
        "  'abre seccion propia para una normativa solo si tienes 2 o mas fragmentos'"
    )
    print(
        "  normativas_detectadas is computed from ANY chunk >= 1 -> mismatch when n=1"
    )
...
def diag_p4(vs: Chroma):
    ...
    query_cases = [
        ...
        (
            "marketplace C2C (probe 7, missing)",
            _make_input(
                tipo_proyecto="ecommerce",
                descripcion_breve="marketplace para vender ropa de segunda mano entre particulares",
                tipos_datos_personales=["nombre", "email", "ubicacion"],
        ...
        (
            "red social UGC (probe 8, missing)",
            _make_input(
                tipo_proyecto="app_web",
        ...
        (
            "plataforma intermediacion directa",
            _make_input(
                tipo_proyecto="ecommerce",
    ...
    for label, inp in query_cases:
        ...
        dsa_hits_above = [
            (d, s) for d, s in dsa_hits if s >= settings.min_relevance_score
        ]
    ...
    out_data = {
        "dsa_scores_by_query": dsa_scores,
        "query_details": results_a,
        "dsa_chunk_sample": chunk_sample,
    }
    out.write_text(
...
def main():
    ...
    parser.add_argument(
        "--p2", action="store_true", help="Diagnose P2 (RGPD no section)"
    )
    parser.add_argument(
        "--p3", action="store_true", help="Diagnose P3 (INJECTION domain gap)"
    )
    parser.add_argument("--p4", action="store_true", help="Diagnose P4 (DSA inverted)")
    parser.add_argument(
        "--all", dest="all_", action="store_true", help="Run all diagnostics"
    )
    ...
    vs = _load_vs()
    print(
        f"threshold={settings.min_relevance_score}  overfetch_k={settings.overfetch_k}"
